var circlecolor=document.getElementsByClassName("circle");//circlecolor is the current color of the circle
var colors=["red","orange","yellow","green","blue","purple"]; //the colors to turn
var colorcount=0;

function colorChange(){
for (var i = 0; i < circlecolor.length; i++) {
circlecolor[i].style.backgroundColor=colors[colorcount];
}
if (colorcount==5) {
colorcount=0;
}
...
document.getElementById("part3clock").innerHTML = minutes + ":" + seconds;

// check if we are above the time limit and set the color of the timer accordingly
if (minutes >= limit) {
  document.getElementById("part3clock").className = "red";
} else {
  document.getElementById("part3clock").className = "blue";
}
